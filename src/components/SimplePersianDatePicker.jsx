...
function SimplePersianDatePicker({ value, onChange, className = "" }) {
    ...
    const gregorianToPersian = (gregorianDate) => {
        ...
        return `${persianYear}/${String(month).padStart(2, "0")}/${String(day).padStart(2, "0")}`;
    };
    ...
    useEffect(() => {
        if (value) {
            setSelectedDate(gregorianToPersian(value));
            const date = new Date(value);
            setCurrentYear(date.getFullYear() - 621);
    ...
    const handleDateSelect = (day) => {
        ...
        if (onChange) {
            const gregorianDate = persianToGregorian(dateStr);
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            days.push(
                <button
                    ...
                    className={`p-2 rounded-lg hover:bg-indigo-500 hover:text-white transition-colors font-medium ${isWeekend ? "text-red-500" : "text-gray-700"
                        }`}
                >
                    {i}
                </button>
    ...
    return (
        <div className="relative" ref={calendarRef}>
            ...
            {showCalendar && (
                <div
                    className={`absolute left-0 ${openUpward ? "bottom-full mb-2" : "top-full mt-2"
